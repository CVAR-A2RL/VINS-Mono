...
int main(int argc, char ** argv)
{
  ...
  auto n = rclcpp::Node::make_shared("data_generator");
  ...
  // Same but in ros2
  auto pub_imu = n->create_publisher<sensor_msgs::msg::Imu>("imu", 1000);
  auto pub_feature = n->create_publisher<sensor_msgs::msg::PointCloud>(
    "/feature_tracker/feature",
    1000);
  auto pub_wifi = n->create_publisher<sensor_msgs::msg::PointCloud>("wifi", 1000);
  auto pub_flow = n->create_publisher<nav_msgs::msg::Odometry>("flow", 1000);
  ...
  // Same but in ros2
  auto pub_path = n->create_publisher<nav_msgs::msg::Path>("path", 1000);
  auto pub_odometry = n->create_publisher<nav_msgs::msg::Odometry>("odometry", 1000);
  ...
  // Same but in ros2
  auto pub_pose = n->create_publisher<geometry_msgs::msg::PoseStamped>("pose", 1000);
  auto pub_cloud = n->create_publisher<sensor_msgs::msg::PointCloud>("cloud", 1000);
  auto pub_ap = n->create_publisher<sensor_msgs::msg::PointCloud>("ap", 1000);
  auto pub_line = n->create_publisher<visualization_msgs::msg::Marker>("sar", 1000);

  // image_transport::ImageTransport it(n);
  // image_transport::Publisher pub_image;
  // pub_image = it.advertise("tracked_image", 1000);
  // Same but in ros2
  auto pub_image = n->create_publisher<sensor_msgs::msg::Image>("tracked_image", 1000);
  ...
  // Same but in ros2
  rclcpp::Rate loop_rate(generator.FREQ);
  ...
  while (rclcpp::ok()) {
    ...
    // Same but in ros2
    double current_time = generator.getTime();
    RCLCPP_INFO(n->get_logger(), "time: %lf", current_time);
    ...
    // Same but in ros2
    for (int i = 0; i < DataGenerator::NUMBER_OF_AP; i++) {
      line_ap[i].header.stamp = rclcpp::Time(current_time);
      line_ap[i].points.back().x = position(0);
      line_ap[i].points.back().y = position(1);
      line_ap[i].points.back().z = position(2);
      pub_line->publish(line_ap[i]);
    ...
    if (publish_count % generator.IMU_PER_WIFI == 0) {
      ...
      // Same but in ros2
      sensor_msgs::msg::PointCloud wifi;
      sensor_msgs::msg::ChannelFloat32 id_ap;
      wifi.header.stamp = rclcpp::Time(current_time);
    ...
    if (publish_count % generator.IMU_PER_IMG == 0) {
      ...
      // Same but in ros2
      sensor_msgs::msg::PointCloud feature;
      sensor_msgs::msg::ChannelFloat32 ids;
      sensor_msgs::msg::ChannelFloat32 pixel;
      sensor_msgs::msg::ChannelFloat32 p_x, p_y, p_z;
      for (int i = 0; i < ROW; i++) {
        for (int j = 0; j < COL; j++) {
          pixel.values.push_back(255);
        }
      ...
      for (auto & id_pts : generator.getImage()) {
        ...
        cv::putText(
          simu_img[id % DataGenerator::NUMBER_OF_CAMERA], label,
          cv::Point2d(p.x + 1, p.y + 1) * 0.5 * 600, cv::FONT_HERSHEY_SIMPLEX, 0.5,
      ...
      for (int i = 0; i < 6; i++) {
        ...
        cv::Point2d origin((generator.output_Axis[i][0].x() + 1) * 300,
          (generator.output_Axis[i][0].y() + 1) * 300);
        cv::Point2d axis_x((generator.output_Axis[i][1].x() + 1) * 300,
          (generator.output_Axis[i][1].y() + 1) * 300);
        cv::Point2d axis_y((generator.output_Axis[i][2].x() + 1) * 300,
          (generator.output_Axis[i][2].y() + 1) * 300);
        cv::Point2d axis_z((generator.output_Axis[i][3].x() + 1) * 300,
          (generator.output_Axis[i][3].y() + 1) * 300);
      ...
      pub_feature->publish(feature);
      RCLCPP_INFO(
        n->get_logger(), "publish image data with stamp %lf",
        feature.header.stamp.sec + feature.header.stamp.nanosec * 1e-9);
