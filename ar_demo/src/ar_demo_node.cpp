...
void axis_generate(visualization_msgs::msg::Marker & line_list, Vector3d & origin, int id)
{
  ...
  line_list.id = id;
  line_list.header.frame_id = "earth";
...
void cube_generate(visualization_msgs::msg::Marker & marker, Vector3d & origin, int id)
{
  ...
  */
  ...
  Cube_corner[id].push_back(
    Vector3d(
      origin.x() - box_length / 2, origin.y() - box_length / 2,
...
void point_callback(const sensor_msgs::msg::PointCloud::SharedPtr point_msg)
{
  ...
  double new_height =
    (height_sum[max_index - 1] + height_sum[max_index] + height_sum[max_index + 1]) / tmp_num;
...
// Same but in ros2
//
bool is_before(const std_msgs::msg::Header & header1, const std_msgs::msg::Header & header2)
{
  auto ts1 = std::chrono::seconds(header1.stamp.sec) + std::chrono::nanoseconds(
    header1.stamp.nanosec);
  auto ts2 = std::chrono::seconds(header2.stamp.sec) + std::chrono::nanoseconds(
    header2.stamp.nanosec);
...
void pose_callback(const nav_msgs::msg::Odometry::SharedPtr pose_msg)
{
  ...
  // Same but in ros2
  while (is_before(img_buf.front()->header, pose_msg->header) && !img_buf.empty()) {
    img_buf.pop();
  }
...
int main(int argc, char ** argv)
{
  ...
  object_pub = node->create_publisher<visualization_msgs::msg::MarkerArray>("AR_object", 10);
  // n.getParam("use_undistored_img", USE_UNDISTORED_IMG);
  // Same but in ros2
  node->get_parameter("use_undistored_img", USE_UNDISTORED_IMG);
  ...
  if (USE_UNDISTORED_IMG) {
    ...
    sub_img = node->create_subscription<sensor_msgs::msg::Image>(
      "image_undistored", 100, img_callback);
  } else {
    ROW = 752;
    COL = 480;
    FOCAL_LENGTH = 460.0;
  ...
  // Same but in ros2
  rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr pose_img;
  ...
  // Same but in ros2
  node->get_parameter("calib_file", calib_file);
  // ROS_INFO("reading paramerter of camera %s", calib_file.c_str());
  // Same but in ros2
  RCLCPP_INFO(node->get_logger(), "reading paramerter of camera %s", calib_file.c_str());
