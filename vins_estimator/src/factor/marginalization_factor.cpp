    ...
    for (int i = 0; i < static_cast<int>(it->parameter_blocks.size()); i++) {
      ...
      for (int j = i; j < static_cast<int>(it->parameter_blocks.size()); j++) {
        ...
        if (i == j) {
          ...
          p->A.block(
            idx_j, idx_i, size_j,
            size_i) = p->A.block(idx_i, idx_j, size_i, size_j).transpose();
        }
      }
  ...
  */
  ...
  for (int i = 0; i < NUM_THREADS; i++) {
    ...
    if (ret != 0) {
      ...
      // ROS_BREAK();
      // Same but in ros2
      RCLCPP_WARN(rclcpp::get_logger("rclcpp"), "pthread_create error");
      rclcpp::shutdown();
    }
  }
  ...
  Eigen::MatrixXd Amm_inv = saes.eigenvectors() * Eigen::VectorXd(
    (saes.eigenvalues().array() > eps).select(
      saes.eigenvalues().array().inverse(),
  ...
  double ** jacobians) const
{
  ...
  for (int i = 0; i < static_cast<int>(marginalization_info->keep_block_size.size()); i++) {
    ...
    if (size != 7) {
      ...
      dx.segment<3>(idx + 3) = 2.0 * Utility::positify(
        Eigen::Quaterniond(
          x0(6), x0(3), x0(4), x0(
            5)).inverse() * Eigen::Quaterniond(x(6), x(3), x(4), x(5))).vec();
      if (!((Eigen::Quaterniond(
          x0(6), x0(3), x0(4),
          x0(5)).inverse() * Eigen::Quaterniond(x(6), x(3), x(4), x(5))).w() >= 0))
      {
  ...
  Eigen::Map<Eigen::VectorXd>(
    residuals,
    n) = marginalization_info->linearized_residuals + marginalization_info->linearized_jacobians *
    dx;
  if (jacobians) {
    ...
    for (int i = 0; i < static_cast<int>(marginalization_info->keep_block_size.size()); i++) {
      if (jacobians[i]) {
        ...
        jacobian.setZero();
        jacobian.leftCols(local_size) = marginalization_info->linearized_jacobians.middleCols(
          idx,
          local_size);
      }
    }
