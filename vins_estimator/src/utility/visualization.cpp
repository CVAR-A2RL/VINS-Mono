...
void printStatistics(const Estimator & estimator, double t)
{
  ...
  printf(
    "position: %f, %f, %f\r", estimator.Ps[WINDOW_SIZE].x(),
  ...
  RCLCPP_DEBUG_STREAM(
    ...
    "position: " << estimator.Ps[WINDOW_SIZE].transpose());
  RCLCPP_DEBUG_STREAM(
    rclcpp::get_logger("rclcpp"),
    "orientation: " << estimator.Vs[WINDOW_SIZE].transpose());
  for (int i = 0; i < NUM_OF_CAM; i++) {
    ...
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger(
        "rclcpp"), "extirnsic tic: " << estimator.tic[i].transpose());
    RCLCPP_DEBUG_STREAM(
      rclcpp::get_logger("rclcpp"),
  ...
  // Same but in ros2
  RCLCPP_DEBUG(rclcpp::get_logger("rclcpp"), "vo solver costs: %f ms", t);
  RCLCPP_DEBUG(
    rclcpp::get_logger(
      "rclcpp"), "average of time %f ms", sum_of_time / sum_of_calculation);

  sum_of_path += (estimator.Ps[WINDOW_SIZE] - last_path).norm();
  last_path = estimator.Ps[WINDOW_SIZE];
  // ROS_DEBUG("sum of path %f", sum_of_path);
  // Same but in ros2
  RCLCPP_DEBUG(rclcpp::get_logger("rclcpp"), "sum of path %f", sum_of_path);
  if (ESTIMATE_TD) {
    // ROS_INFO("td %f", estimator.td);
    // Same but in ros2
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "td %f", estimator.td);
  }
}
...
void pubTF(const Estimator & estimator, const std_msgs::msg::Header & header)
{
  ...
  transform.transform.rotation.x = ric_q.x();
  transform.transform.rotation.y = ric_q.y();
  transform.transform.rotation.z = ric_q.z();
  transform.transform.rotation.w = ric_q.w();
  tf_broadcaster->sendTransform(transform);
