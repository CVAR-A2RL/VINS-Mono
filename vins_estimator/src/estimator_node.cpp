...
// Same but in ros2
std::vector<std::pair<std::vector<sensor_msgs::msg::Imu::SharedPtr>,
...
getMeasurements()
{
  ...
  while (true) {
    ...
    if (!(imu_buf.back()->header.stamp.sec + imu_buf.back()->header.stamp.nanosec * 1e-9 >
      feature_buf.front()->header.stamp.sec + feature_buf.front()->header.stamp.nanosec * 1e-9 +
      estimator.td))
    {
      ...
      estimator.td))
    {
      ...
      // Same but in ros2
      RCLCPP_WARN(rclcpp::get_logger("rclcpp"), "throw img, only should happen at the beginning");
      feature_buf.pop();
      continue;
    }
    ...
    while (imu_buf.front()->header.stamp.sec + imu_buf.front()->header.stamp.nanosec * 1e-9 <
      img_msg->header.stamp.sec + img_msg->header.stamp.nanosec * 1e-9 + estimator.td)
    {
    ...
    if (IMUs.empty()) {
      ...
      // Same but in ros2
      RCLCPP_WARN(rclcpp::get_logger("rclcpp"), "no imu between two image");
    }
...
void imu_callback(const sensor_msgs::msg::Imu::SharedPtr imu_msg)
{
  if (imu_msg->header.stamp.sec + imu_msg->header.stamp.nanosec * 1e-9 <= last_imu_t) {
    ...
    // Same but in ros2
    RCLCPP_WARN(rclcpp::get_logger("rclcpp"), "imu message in disorder!");
    return;
  }
...
void restart_callback(const std_msgs::msg::Bool::SharedPtr restart_msg)
{
  if (restart_msg->data == true) {
    ...
    // Same but in ros2
    RCLCPP_WARN(rclcpp::get_logger("rclcpp"), "restart the estimator!");
    m_buf.lock();
  ...
  while (true) {
    ...
    for (auto & measurement : measurements) {
      ...
      for (auto & imu_msg : measurement.first) {
        double t = imu_msg->header.stamp.sec + imu_msg->header.stamp.nanosec * 1e-9;
        double img_t = img_msg->header.stamp.sec + imu_msg->header.stamp.nanosec * 1e-9 +
          estimator.td;
        if (t <= img_t) {
          if (current_time < 0) {
            current_time = t;
      ...
      RCLCPP_DEBUG(
        rclcpp::get_logger(
          "rclcpp"), "processing vision data with stamp %f \n",
