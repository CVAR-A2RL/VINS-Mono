...
// Same but in ros2
std::queue<sensor_msgs::msg::Image::SharedPtr> image_buf;
std::queue<sensor_msgs::msg::PointCloud::SharedPtr> point_buf;
std::queue<nav_msgs::msg::Odometry::SharedPtr> pose_buf;
...
void new_sequence()
{
  ...
  if (sequence > 5) {
    ...
    RCLCPP_WARN(
      rclcpp::get_logger(
        "rclcpp"), "only support 5 sequences since it's boring to copy code for more sequences.");
    // rclcpp::shutdown();
  }
...
void image_callback(const sensor_msgs::msg::Image::SharedPtr image_msg)
{
  ...
  } else if (image_msg->header.stamp.sec - last_image_time > 1.0 ||
    image_msg->header.stamp.sec < last_image_time)
  {
    // ROS_WARN("image discontinue! detect a new sequence!");
    // new_sequence();
    // Same but in ros2
    RCLCPP_WARN(rclcpp::get_logger("rclcpp"), "image discontinue! detect a new sequence!");
    new_sequence();
  }
  last_image_time = image_msg->header.stamp.sec;
}
...
// Same but in ros2
void imu_forward_callback(const nav_msgs::msg::Odometry::SharedPtr forward_msg)
{
...
// Same but in ros2
void relo_relative_pose_callback(const nav_msgs::msg::Odometry::SharedPtr pose_msg)
{
  Vector3d relative_t = Vector3d(
    pose_msg->pose.pose.position.x,
    pose_msg->pose.pose.position.y,
    pose_msg->pose.pose.position.z);
...
void process()
{
  ...
  while (true) {
    ...
    if (!image_buf.empty() && !point_buf.empty() && !pose_buf.empty()) {
      if (
        image_buf.front()->header.stamp.sec + image_buf.front()->header.stamp.nanosec * 1e-9 >
        ...
        point_buf.front()->header.stamp.sec + point_buf.front()->header.stamp.nanosec * 1e-9)
      {
        ...
        image_buf.back()->header.stamp.sec + image_buf.back()->header.stamp.nanosec * 1e-9 >=
        pose_buf.front()->header.stamp.sec + pose_buf.front()->header.stamp.nanosec * 1e-9 &&
...
int main(int argc, char ** argv)
{
  ...
  // Same but in ros2
  auto sub_imu_forward = n->create_subscription<nav_msgs::msg::Odometry>(
    "/vins_estimator/imu_propagate", 2000, imu_forward_callback);
