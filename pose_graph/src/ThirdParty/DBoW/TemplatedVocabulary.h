...
// --------------------------------------------------------------------------
  ...
  F > ::TemplatedVocabulary(const std::string & filename) : m_scoring_object(NULL)
  {
    //m_scoring = KL;
    // Changed by VINS [[[
    //printf("loop start load bin\n");
...
// --------------------------------------------------------------------------
  ...
  template < class TDescriptor, class F >
  TemplatedVocabulary < TDescriptor,
  F > ::TemplatedVocabulary(const char * filename) : m_scoring_object(NULL)
  {
    //m_scoring = KL;
    // Changed by VINS [[[
    //printf("loop start load bin\n");
...
// --------------------------------------------------------------------------
  ...
  TemplatedVocabulary < TDescriptor, F > ::operator =
    (const TemplatedVocabulary < TDescriptor, F > &voc)
  {
    std::cout << "voc m_k: " << voc.m_k << std::endl;
    std::cout << "voc m_L: " << voc.m_L << std::endl;
    this->m_k = voc.m_k;
    this->m_L = voc.m_L;
...
// --------------------------------------------------------------------------
    ...
    const std::vector < std::vector < TDescriptor >> &training_features)
  {
    ...
    std::cout << "reserve 1 size: " << expected_nodes << std::endl;
    m_nodes.reserve(expected_nodes); // avoid allocations when creating the tree
    std::cout << "done reserve" << std::endl;
...
// --------------------------------------------------------------------------
    ...
    std::vector < pDescriptor > &features) const
  {
    ...
    for (vvit = training_features.begin(); vvit != training_features.end(); ++vvit) {
      std::cout << "reserve 2 size: " << features.size() + vvit->size() << std::endl;
...
// --------------------------------------------------------------------------
    ...
    const std::vector < pDescriptor > &descriptors, int current_level)
  {
    ...
    std::cout << "reserve 3 size: " << m_k << std::endl;
    clusters.reserve(m_k);
    std::cout << "done reserve" << std::endl;
    std::cout << "reserve 4 size: " << m_k << std::endl;
    groups.reserve(m_k);
    std::cout << "done reserve" << std::endl;
      ...
      while (goon) {
        ...
        if (first_time) {
          ...
          for (unsigned int c = 0; c < clusters.size(); ++c) {
            ...
            std::cout << "reserve 5 size: " << groups[c].size() << std::endl;
    ...
    if (current_level < m_L) {
      ...
      for (unsigned int i = 0; i < clusters.size(); ++i) {
        ...
        std::cout << "reserve 6 size: " << groups[i].size() << std::endl;
...
// --------------------------------------------------------------------------
    ...
    std::vector < TDescriptor > &clusters) const
  {
    ...
    clusters.resize(0);
    std::cout << "reserve 7 size: " << clusters.size() << std::endl;
    clusters.reserve(m_k);
    std::cout << "done reserve" << std::endl;
...
// --------------------------------------------------------------------------
  ...
  void TemplatedVocabulary < TDescriptor, F > ::createWords()
  {
    ...
    if (!m_nodes.empty()) {
      std::cout << "reserve 8 size: " << (int)pow((double)m_k, (double)m_L) << std::endl;
...
// --------------------------------------------------------------------------
  ...
  template < class TDescriptor, class F >
  void TemplatedVocabulary < TDescriptor,
  F > ::transform(const TDescriptor & feature, WordId & id) const
  {
    WordValue weight;
...
// --------------------------------------------------------------------------
    ...
    &words) const
  {
    ...
    if (m_nodes[nid].isLeaf()) {
      ...
      std::cout << "reserve 9 size: " << m_k << std::endl;
