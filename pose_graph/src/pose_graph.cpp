...
int PoseGraph::detectLoop(KeyFrame * keyframe, int frame_index)
{
  ...
  if (DEBUG_IMAGE) {
    ...
    putText(
      compressed_image, "feature_num:" + to_string(feature_num), cv::Point2f(
        10,
        10), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(
        255));
  ...
  if (DEBUG_IMAGE) {
    ...
    if (ret.size() > 0) {
      putText(
        loop_result, "neighbour score:" + to_string(ret[0].Score), cv::Point2f(
          10,
          50), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(
          255));
    }
  }
  // visual loop result
  if (DEBUG_IMAGE) {
    for (unsigned int i = 0; i < ret.size(); i++) {
      ...
      putText(
        tmp_image, "index:  " + to_string(tmp_index) + "loop score:" + to_string(
          ret[i].Score), cv::Point2f(10, 50), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255));
    ...
    for (unsigned int i = 1; i < ret.size(); i++) {
      ...
      if (ret[i].Score > 0.015) {
        ...
        if (DEBUG_IMAGE && 0) {
          ...
          cv::Mat tmp_image = (it->second).clone();
          putText(
            tmp_image, "loop score:" + to_string(ret[i].Score), cv::Point2f(
              10,
              50), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(
              255));
          cv::hconcat(loop_result, tmp_image, loop_result);
        }
      }
        ...
        for (int j = 1; j < 5; j++) {
          if (i - j >= 0 && sequence_array[i] == sequence_array[i - j]) {
            ...
            Vector3d relative_t(t_array[i][0] - t_array[i - j][0],
              t_array[i][1] - t_array[i - j][1], t_array[i][2] - t_array[i - j][2]);
      ...
      &keypoints_num) != EOF)
  {
    ...
    */
    ...
    KeyFrame * keyframe = new KeyFrame(
      time_stamp, index, VIO_T, VIO_R, PG_T, PG_R, image,
...
void PoseGraph::publish()
{
  ...
  base_path.header.frame_id = "earth";
