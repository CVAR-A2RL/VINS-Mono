  ...
  const std::vector<cv::KeyPoint> & keypoints_old_norm)
{
  for (int i = 0; i < (int)window_brief_descriptors.size(); i++) {
    ...
    if (searchInAera(
        window_brief_descriptors[i], descriptors_old, keypoints_old,
  ...
  Eigen::Vector3d & PnP_T_old, Eigen::Matrix3d & PnP_R_old)
{
  ...
  if (CV_MAJOR_VERSION < 3) {
    solvePnPRansac(
      matched_3d, matched_2d_old_norm, K, D, rvec, t, true, 100, 10.0 / 460.0, 100,
      inliers);
  } else {
    if (CV_MINOR_VERSION < 2) {
      solvePnPRansac(
        matched_3d, matched_2d_old_norm, K, D, rvec, t, true, 100, sqrt(
          10.0 / 460.0), 0.99, inliers);
    } else {
      solvePnPRansac(
        matched_3d, matched_2d_old_norm, K, D, rvec, t, true, 100, 10.0 / 460.0, 0.99,
        inliers);
    }
...
bool KeyFrame::findConnection(KeyFrame * old_kf)
{
  ...
  searchByBRIEFDes(
    matched_2d_old, matched_2d_old_norm, status, old_kf->brief_descriptors,
  ...
  */
  ...
  if ((int)matched_2d_cur.size() > MIN_LOOP_NUM) {
    ...
    if (DEBUG_IMAGE) {
      ...
      putText(
        notation, "current frame: " + to_string(index) + "  sequence: " + to_string(
          sequence), cv::Point2f(20, 30), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255), 3);

      putText(
        notation,
        "previous frame: " + to_string(old_kf->index) + "  sequence: " + to_string(
          old_kf->sequence), cv::Point2f(20 + COL + gap, 30), cv::FONT_HERSHEY_SIMPLEX, 1,
        cv::Scalar(255), 3);
      ...
      if ((int)matched_2d_cur.size() > MIN_LOOP_NUM) {
        ...
        //       Same but in ros2
        sensor_msgs::msg::Image msg = *cv_bridge::CvImage(
          std_msgs::msg::Header(), "bgr8", thumbimage).toImageMsg();
